...
typedef struct {
  ...
  unsigned short listen[MAX_CONNECTIONS];
...
typedef struct {
  unsigned char bitfield[5]; //bitfield where each bit represents a piece that
                   //the peer has or doesn't have
  int size;//size of the bitfiled
} bt_bitfield_t;
...
typedef struct{
  int index; //which piece index
  int begin; //offset within piece
  char piece[256]; //pointer to start of the data for a piece
  int length;
} bt_piece_t;
...
int save_piece(bt_args_t * bt_args, bt_msg *bt_msg,SHA_CTX *c, std::ofstream& ofile);

/*load a piece of the file into piece */
//int load_piece(bt_args_t * bt_args, bt_piece_t * piece);
int load_piece(bt_args_t * bt_args, bt_msg *bt_msg, bt_piece_t *piece, int *length);

/*load the bitfield into bitfield*/
int get_bitfield(bt_args_t * bt_args, bt_bitfield_t * bitfield);

void delete_piece(bt_msg_t *bt_msg, bt_args_t *bt_args);
...
int createHndshkMsg(char *handshakemsg,sockaddr_in clientAddr,char *infohash);
